...
use idevice::{IdeviceError, IdeviceService, RsdService, amfi::AmfiClient, provider::IdeviceProvider};
...
/// `client` must be a valid, non-null pointer to a location where the handle will be stored
#[unsafe(no_mangle)]
pub unsafe extern "C" fn amfi_connect_rsd(
    ...
    let res: Result<AmfiClient, IdeviceError> = run_sync_local(async move {
        ...
        AmfiClient::connect_rsd(provider_ref, handshake_ref).await
    });
