...
use idevice::{
    IdeviceError, IdeviceService, RsdService, companion_proxy::CompanionProxy,
...
/// `client` must be a valid, non-null pointer to a location where the handle will be stored
#[unsafe(no_mangle)]
pub unsafe extern "C" fn companion_proxy_connect_rsd(
    ...
    let res: Result<CompanionProxy, IdeviceError> = run_sync_local(async move {
        ...
        CompanionProxy::connect_rsd(provider_ref, handshake_ref).await
    });
